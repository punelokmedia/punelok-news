...
export default function PoliticsNewsPage() {
    ...
    useEffect(() => {
        const fetchNews = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/news?category=politics&language=${language}`);
                setNews(res.data);
            } catch (error) {
                console.error("Failed to fetch politics news", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const title = language === 'marathi' ? 'राजकारण' : language === 'hindi' ? 'राजनीति' : 'Politics';
                        ...
                        {news.map((item) => (
                            <Link href={`/news/${item._id}`} key={item._id} className="group flex flex-col no-underline">
                                <div className="relative aspect-video rounded-xl overflow-hidden mb-4 shadow-sm group-hover:shadow-md transition-shadow">
                                    <img 
                                        src={item.image || 'https://placehold.co/600x400/png?text=Politics'} 
